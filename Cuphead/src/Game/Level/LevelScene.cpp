...
namespace Cuphead
{
	...
	{
		Teddy::Ref<Teddy::Scene> newScene = Teddy::CreateRef<Teddy::Scene>(true);

		auto camEntt = newScene->CreateEntity("Main Menu Camera");
		...
		newScene->OnViewportResize(window.GetWidth(), window.GetHeight());
		cam.Camera.GetWidthAndHeight(m_WorldWidth, m_WorldHeight);

		auto& assets = Teddy::AssetManager::Get();

		m_Background.LeftBackground = newScene->CreateEntity("Main Menu Background");
		...
		m_Background.RightBackground = newScene->CreateEntity("Main Menu Background");
		...
		// TODO: Textures not unloading from other scenes
		m_Background.Spire = newScene->CreateEntity("Phase 1 Spire");
